...
Never touch server/, pipeline/, database schema, or migration files. Those belong to the backend developer. If a task requires a new endpoint or a change to the API, build the frontend against a mock that matches the expected shape and flag what the backend developer needs to provide.
